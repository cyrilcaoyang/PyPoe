...
@main.command()
def debug_linux():
    """Debug Linux-specific history manager issues."""
    import subprocess
    import sys
    from pathlib import Path
    
    script_path = Path(__file__).parent / "scripts" / "debug_linux_history.py"
    if not script_path.exists():
        click.echo("❌ Debug script not found")
        return
    
    try:
        subprocess.run([sys.executable, str(script_path)], check=True)
    except subprocess.CalledProcessError as e:
        click.echo(f"❌ Debug script failed: {e}")

@main.command()
def troubleshoot_linux():
    """Troubleshoot and fix Linux-specific history manager issues."""
    import subprocess
    import sys
    from pathlib import Path
    
    script_path = Path(__file__).parent / "scripts" / "linux_history_troubleshoot.py"
    if not script_path.exists():
        click.echo("❌ Troubleshoot script not found")
        return
    
    try:
        subprocess.run([sys.executable, str(script_path)], check=True)
    except subprocess.CalledProcessError as e:
        click.echo(f"❌ Troubleshoot script failed: {e}")
